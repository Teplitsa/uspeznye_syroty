...
if ( ! function_exists( 'presscore_enqueue_web_fonts' ) ) :
	...
	function presscore_enqueue_web_fonts() {
		...
		foreach ( $options as $option ) {
			...
			$font = of_get_option( $option['id'] );

			// Add bold to basic font.
			if ( 'fonts-font_family' === $option['id'] ) {
                $font_obj = new Presscore_Web_Font( $font );
                if ( '700' != $font_obj->get_weight() ) {
                    $font_obj->add_weight( '700' );
                    $font = (string) $font_obj;
                }
            }
